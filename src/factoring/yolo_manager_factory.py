...
class YoloManagerFactory(DLManagerFactory):
    ...
    @staticmethod
    def get_dl_manager(model_path: str, config: dict = None) -> DLModelManagerAbstract:
        if config is not None and "imgsz" in config:
            config["imgsz"] = tuple(config["imgsz"])

        return YoloModelManager(model_path, config)
    ...
    @staticmethod
    def get_final_dl_manager(model_path: str, config: dict = None) -> DLModelManagerAbstract:
        return YoloManagerFactory.get_logging_decorator(
            YoloManagerFactory.get_data_preparator(YoloManagerFactory.get_dl_manager(model_path, config)))
